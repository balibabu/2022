...
class Dino:
    ...
    def gameOver(screen):
        ...
        screen.text(f"Your Score:{Dino.score//len(Dino.newObstaclePoints)}",10,38,0)
        screen.show()
    ...
    def play(btnPin):
        ...
        while True:
            if c==0: # half screen +- random between 1 to 10
                Dino.obstaclePoints+=Dino.newObstaclePoints
            c=(c+Dino.obSpeed)%60

            if button.value()==1:
                jump(screen,Dino.dinoPoints,Dino.obstaclePoints,y=-2)
                jump(screen,Dino.dinoPoints,Dino.obstaclePoints,y=2)
            else:
                Dino.render(screen,Dino.dinoPoints+Dino.obstaclePoints)
                screen.show()
                Dino.render(screen,Dino.dinoPoints+Dino.obstaclePoints,0)
                Dino.move(Dino.obstaclePoints,x=-Dino.obSpeed)
            if Dino.condition(Dino.dinoPoints,Dino.obstaclePoints): break
            Dino.removeOutlier()
